/**
 * @file expect.h
 * @author Matthew Weissel (null@mattweissel.info)
 * @brief Utility functions for evaluating test success.
 */
#ifndef EXPECT_H
#define EXPECT_H

#include "common.h"

#include "core/logger.h"
#include "math/math.h"

/**
 * @brief Truth test predicate.
 */
#define EXPECT(p)                                       \
    if ( (p) != true )                                  \
    {                                                   \
        LOGERROR ( "--> Expected true, tested false.\n" \
                   "            in file:\t%s:%d."       \
                 , __FILE__ , __LINE__                  \
                 );                                     \
        return false;                                   \
    }

/**
 * @brief Falsehood test predicate.
 */
#define EXPECT_NOT(p)                                   \
    if ( (p) != false )                                 \
    {                                                   \
        LOGERROR ( "--> Expected false, tested true.\n" \
                   "            in file:\t%s:%d."       \
                 , __FILE__ , __LINE__                  \
                 );                                     \
        return false;                                   \
    }

/**
 * @brief Equality test predicate.
 */
#define EXPECT_EQ(expected,actual)                              \
    if ( (actual) != (expected) )                               \
    {                                                           \
        LOGERROR ( "--> Expected %lld, tested %lld.\n"          \
                   "            in file:\t%s:%d."               \
                 , (expected) , (actual) , __FILE__ , __LINE__  \
                 );                                             \
        return false;                                           \
    }

/**
 * @brief Inequality test predicate.
 */
#define EXPECT_NEQ(expected,actual)                                         \
    if ( (actual) == (expected) )                                           \
    {                                                                       \
        LOGERROR ( "--> Expected something other than %lld, tested %lld.\n" \
                   "            in file:\t%s:%d."                           \
                 , (expected) , (actual) , __FILE__ , __LINE__              \
                 );                                                         \
        return false;                                                       \
    }

/**
 * @brief Tests floats within a tolerance of K_FLOAT_EPSILON.
 *        (see math/float.h)
 */
#define EXPECT_EQF(expected,actual,tolerance)                   \
    if ( abs ( (expected) - (actual) ) > (tolerance) )          \
    {                                                           \
        LOGERROR ( "--> Expected %f, received %f.\n"            \
                   "            in file:\t%s:%d."               \
                 , (expected) , (actual) , __FILE__ , __LINE__  \
                 );                                             \
        return false;                                           \
    }




#endif  // EXPECT_H
